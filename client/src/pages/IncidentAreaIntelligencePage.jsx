...
const IncidentAreaIntelligencePage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setMessage("");
      try {
        const [profileRes, institutionsRes, facilitiesRes, incidentsRes] = await Promise.all([
          apiFetch(`${ACCOUNTS_API}/profile/`, { headers }),
          apiFetch(`${ACCOUNTS_API}/institutions/`, { headers }),
          apiFetch(`${SECURITY_API}/facilities/`, { headers }),
          apiFetch(`${INCIDENTS_API}/`, { headers }),
        ]);

        const [profileData, institutionsData, facilitiesData, incidentsData] = await Promise.all([
          profileRes.json(),
  ...
  return (
    <div style={{ ...styles.page, backgroundColor: theme.pageBg }}>
      <div style={styles.header}>
        <div>
          <h1 style={{ ...styles.title, color: theme.text }}>Area Intelligence</h1>
          <p style={{ ...styles.subtitle, color: theme.mutedText }}>
            Drag the ring, change the radius, and generate an AI operations brief for facilities and incidents inside the area.
          </p>
        </div>
      ...
      <section style={styles.workspace}>
        ...
        <div style={styles.sideStack}>
          <section style={styles.sideCard}>
            <h2 style={styles.cardTitle}>AI operations brief</h2>
            {!analysisResult ? (
              <p style={styles.muted}>Run the analysis to get deployment posture, facility observations, and recommended actions.</p>
            ) : (
              <div style={styles.analysisStack}>
                ...
                <div style={styles.analysisBlock}>
                  <div style={styles.analysisLabel}>Facility observations</div>
                  {(analysisResult.analysis?.facility_observations || []).map((item) => (
                    <div key={item} style={styles.analysisText}>{item}</div>
                  ))}
                </div>
                <div style={styles.analysisBlock}>
                  <div style={styles.analysisLabel}>Incident observations</div>
                  {(analysisResult.analysis?.incident_observations || []).map((item) => (
                    <div key={item} style={styles.analysisText}>{item}</div>
                  ))}
                </div>
                <div style={styles.analysisBlock}>
                ...
                {analysisResult.meta?.used_fallback ? <div style={styles.helperText}>Using built-in fallback analysis.</div> : null}
              </div>
            )}
          </section>
...
export default IncidentAreaIntelligencePage;
